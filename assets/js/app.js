(function ($) {
  ...
  $(".sidebar-menu .dropdown").on("click", function () {
    var item = $(this);
    item.siblings(".dropdown").children(".sidebar-submenu").slideUp(0);

    item.siblings(".dropdown").removeClass("dropdown-open");

    item.siblings(".dropdown").removeClass("open");

    item.children(".sidebar-submenu").slideToggle(0);
  ...
  function updateButton({ buttonEl, isDark }) {
    ...
    const newCta = isDark != true ? "dark" : "light";
    // use an aria-label if you are omitting text on the button
    // and using a sun/moon icon, for example
    if (buttonEl != null && newCta != null) {
  ...
  function updateThemeOnHtmlEl({ theme }) {
    document.getElementById("themebutton").setAttribute("aria-label", theme);
...
if (!localStorageTheme) {
    localStorage.setItem("theme", "light"); // Ensure "light" is stored if null
